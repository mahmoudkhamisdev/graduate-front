    ...
    <div className="w-full max-w-sm mx-auto space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <CustomInput id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
        </div>
          ...
          <div className="flex items-center gap-2 justify-between flex-wrap !-mt-px">
            <div className="flex items-center space-x-2">
              ...
              <div className="grid gap-1.5 leading-none">
                ...
                >
                  <TranslatableText text="Remember me" />
                </label>
              </div>
            </div>

            <div className="text-end">
              <LinkButton type="button" className="p-0" onClick={() => navigate("/forgot-password")}>
                <TranslatableText text="Forgot your password?" />
              </LinkButton>
            </div>
          </div>
        </div>
        ...
        {/* Submit */}
        <MainButton type="submit" className="w-full h-11 text-lg" disabled={loading}>
          {loading && <Loader className="animate-spin" />}
          <TranslatableText text="Continue With Email" />
        </MainButton>

        <div className="flex items-center gap-1 justify-center">
          <span className="bg-muted-foreground h-px flex-1" /> <TranslatableText text="Or continue with" />
          <span className="bg-muted-foreground h-px flex-1" />
        </div>
        <SecondaryButton className="w-full h-10" type="button" onClick={handleGoogle} disabled={loading}>
          {GoogleSVG}

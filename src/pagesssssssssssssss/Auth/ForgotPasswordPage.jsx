...
import { Snowflake, ArrowLeft, KeyRound, CheckCircle2, EyeOff, Eye } from "lucide-react"
...
export function ForgotPasswordPage({ onComplete }) {
  ...
  const handleKeyDown = (i, e) => {
    if (e.key === "Backspace" && !verificationCode[i] && i > 0) inputRefs.current[i - 1]?.focus()
  ...
  const handleVerifyCode = async () => {
    if (verificationCode.some((d) => !d)) return setVerificationError("Enter all 6 digits")

    try {
      setIsSubmitting(true)
      const { data } = await axios.post(`${BaseUrlApi}/auth/verify-reset-code`, {
        email,
  ...
  const handleResetSubmit = async (e) => {
    e.preventDefault()
    if (!password || !confirmPassword) return setResetError("All fields are required")
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white">
            {step === "email" && <TranslatableText text="Forgot your password?" />}
            {step === "verification" && <TranslatableText text="Verify your email" />}
            {step === "reset" && <TranslatableText text="Reset your password" />}
            {step === "success" && <TranslatableText text="Success!" />}
          </h2>
          <p className="text-muted-foreground ">
            {step === "email" && <TranslatableText text="We'll send you a verification code." />}
            {step === "verification" && <TranslatableText text="Enter the 6-digit code sent to your email." />}
            {step === "reset" && <TranslatableText text="Choose a new password and confirm it below." />}
            {step === "success" && <TranslatableText text="Your password has been reset." />}
          </p>
        </div>
      </div>
      ...
      >
        ...
        {step === "email" && (
          <form onSubmit={handleEmailSubmit} className="space-y-6">
            ...
            <MainButton type="submit" className="w-full h-11 text-lg" disabled={isSubmitting}>
              {isSubmitting ? (
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <p className="text-center text-muted-foreground  mb-4">
                <TranslatableText text="We've sent a code to" /> <span className="text-main">{email}</span>
              </p>
              ...
              {verificationError && (
                <p className="text-red-500 text-sm text-center mt-2">
                  <TranslatableText text={verificationError} />
                </p>
              )}

              <div className="text-center mt-2">
                <LinkButton onClick={handleEmailSubmit} className="text-sm">
                  <TranslatableText text="Didn't receive a code? Resend" />
                </LinkButton>
              </div>
            </div>

            <MainButton onClick={handleVerifyCode} className="w-full h-11 text-lg" disabled={isSubmitting}>
              {isSubmitting ? <TranslatableText text="Verifying..." /> : <TranslatableText text="Verify Code" />}
            </MainButton>
          </div>
        )}
          ...
          >
            <div className="space-y-2 relative">
              <label htmlFor="new-password" className="text-white">
                <TranslatableText text="New Password" />
              </label>
              <CustomInput
                id="new-password"
            ...
            {resetError && (
              <p className="text-red-500 text-sm">
                <TranslatableText text={resetError} />
              </p>
            )}

            <MainButton type="submit" className="w-full h-11 text-lg rounded-lg" disabled={isSubmitting}>
              {isSubmitting ? <TranslatableText text="Resetting…" /> : <TranslatableText text="Reset Password" />}
            </MainButton>
          </form>
        )}
        ...
        {step === "success" && (
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-white text-center">
              <TranslatableText text="Password Reset Successful!" />
            </h3>
            <p className="text-center text-muted-foreground  max-w-sm leading-relaxed">
              <TranslatableText text="You've successfully reset your password. Click below to return to login." />
            </p>
            <MainButton onClick={() => navigate("/")} className="w-full h-11 rounded-xl text-lg font-medium">
              <TranslatableText text="Return to Login" />
            </MainButton>
          </motion.div>
        )}
      </motion.div>
